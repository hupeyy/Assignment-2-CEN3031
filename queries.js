 ...
 */
  ...
  /*In this file - queries.js - You will define several queries that retreive, remove, add, and update listing entries in the database. 
    ...
    Security Best Practices: Use a tool to monitor vulnerabilities and to scan your code for vulnerabilities.
   ...
   */
    ...
    */
      async function removeCable() {
        ...
        if (cable === null) {
          console.log('CABL BLDG has been removed.');
        } else {
          console.log('CABL BLDG has not been removed.');
        }
    }
    ...
    */
    async function updatePhelpsLab() {
       //ADD CODE HERE
      const old_address = await Listing.findOne({where: {name: 'Phelps Laboratory'}});
      const new_address = await Listing.update({address: '1953 Museum Rd, Gainesville, FL 32603'}, {where: {name: 'Phelps Laboratory'}});
      console.log('UpdatingPhelpsLab.');
      console.log("Old: ", old_address);
      console.log("New: ", new_address);
    }
   ...
   testQueries().catch((error) => {
     console.error('Unable to test queries:', error);
   });